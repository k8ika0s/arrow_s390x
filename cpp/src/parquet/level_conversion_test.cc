...
TEST(GreaterThanBitmap, GeneratesExpectedBitmasks) {
  std::vector<int16_t> levels = {0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7,
                                 ...
                                 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7,
                                 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7};
  auto gt_bitmap = [&](int64_t num_levels, int16_t rhs) {
    return ::arrow::bit_util::FromLittleEndian(
        GreaterThanBitmap(levels.data(), num_levels, rhs));
  };
  EXPECT_EQ(gt_bitmap(/*num_levels=*/0, /*rhs=*/0), 0);
  EXPECT_EQ(gt_bitmap(/*num_levels=*/64, /*rhs=*/8), 0);
  EXPECT_EQ(gt_bitmap(/*num_levels=*/64, /*rhs=*/-1),
            0xFFFFFFFFFFFFFFFF);
  // Should be zero padded.
  EXPECT_EQ(gt_bitmap(/*num_levels=*/47, /*rhs=*/-1),
            0x7FFFFFFFFFFF);
  EXPECT_EQ(gt_bitmap(/*num_levels=*/64, /*rhs=*/6),
            0x8080808080808080);
}
